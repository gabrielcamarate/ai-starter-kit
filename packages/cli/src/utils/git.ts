...
function commitWithMessage(cwd: string, subject: string, changedBullet: string, whyBullet: string): void {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "askit-commit-"));
  const messageFile = path.join(tempDir, "commit-message.txt");

  const lines = [subject, "", "What changed:", `- ${changedBullet}`, "", "Why:", `- ${whyBullet}`, ""];
  fs.writeFileSync(messageFile, lines.join(os.EOL), "utf-8");

  const result = run("git", ["commit", "-F", messageFile], cwd);
...
export function runGitInitWithTwoCommits(input: {
  ...
  commitWithMessage(
    ...
    "added the base structure (agents/controls/skills/docs/security), policies, and governance files.",
    "establish a consistent and secure baseline for all projects."
  );
  ...
  commitWithMessage(
    ...
    `added ${input.template} stack files and minimum development/quality scripts.`,
    "enable a fast start with a consistent and functional stack."
  );
