...
export async function runNewAgentCommand(name: string, options: GeneratorOptions): Promise<void> {
  const safeName = normalizeName(name);
  const root = options.path ? path.resolve(options.path) : process.cwd();
  const target = path.join(root, "agents", `${safeName}.md`);
  const scaffold = fs.readFileSync(path.join(SCAFFOLDS_PATH, "agent.md"), "utf-8");
  const content = scaffold.replaceAll("{{name}}", safeName);

  writeFileExclusive(target, content);
  logInfo(`Agent criado: ${target}`);
}
