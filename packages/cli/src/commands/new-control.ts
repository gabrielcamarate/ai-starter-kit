...
export async function runNewControlCommand(name: string, options: GeneratorOptions): Promise<void> {
  const safeName = normalizeName(name);
  const root = options.path ? path.resolve(options.path) : process.cwd();
  const target = path.join(root, "controls", `${safeName}.md`);
  const scaffold = fs.readFileSync(path.join(SCAFFOLDS_PATH, "control.md"), "utf-8");
  const content = scaffold.replaceAll("{{name}}", safeName);

  writeFileExclusive(target, content);
  logInfo(`Control criado: ${target}`);
}
